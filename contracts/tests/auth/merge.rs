...
use simplex::transaction::{FinalTransaction, PartialOutput, utxo::UTXO};
...
fn merge_transaction(
    ...
    for utxo in utxos {
        fixture.add_storm_eye_input(
            ...
            utxo,
            AuthSpendPath::Merge {
                utxos_to_merge: declared_count,
            },
        );
    }
    fixture.add_storm_eye_outputs(&mut tx, &[output_amount]);
...
fn rejects_merge_of_a_single_utxo(context: simplex::TestContext) -> anyhow::Result<()> {
    ...
    let tx = merge_transaction(&fixture, &utxos, 1, total(&utxos));
    assert_covenant_rejects(&context, &tx);
...
fn rejects_merge_that_does_not_conserve_value(context: simplex::TestContext) -> anyhow::Result<()> {
    ...
    let tx = merge_transaction(&fixture, &utxos, utxos.len() as u8, total(&utxos) - 1_000);
    assert_covenant_rejects(&context, &tx);
...
fn rejects_merge_ignoring_an_extra_storm_eye_input(
    ...
    for utxo in &utxos {
        fixture.add_storm_eye_input(&mut tx, utxo, AuthSpendPath::Merge { utxos_to_merge: 2 });
    }
    fixture.add_storm_eye_outputs(&mut tx, &[counted]);
