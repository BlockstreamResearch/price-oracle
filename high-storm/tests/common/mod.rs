//! A live Oracle Network, in process.
//!
//! Every node is a real `HighStorm` speaking the real Storm protocol over loopback TCP
//! with an in-memory SQLite store, so signatures really are produced by a MuSig2 session
//! between separate nodes. No Docker and no Postgres are involved.
