...
/// Local hex, so this test needs no extra dependency.
...
/// The first `n` bits of a key, MSB first — this is the path monotree walks.
...
///
/// The child hashes sit on the outskirts of the serialization by design, so there are only
/// ever two possible insertion points: offset 0, or just before the trailing type flag.
